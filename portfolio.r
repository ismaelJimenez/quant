...
efficient.portfolio.risk <-
  function(er, cov.mat, target.sd, shorts=TRUE)
  {
    # compute maximum return portfolio subject to target sd
    #
    # inputs:
    # er					    N x 1 vector of expected returns
    # cov.mat  			  N x N covariance matrix of returns
    # target.sd	      scalar, target expected sd
    ...
    #
    ef <- globalMin.portfolio(er, covmat, shorts)
    
    div <- 0.01
    target_er <- ef$er
    
    while(div >= 0.0001){
      while(ef$sd <= target.sd) {
        target_er <- target_er+div
        ef <-efficient.portfolio(er, covmat, target_er, shorts)
      }
      target_er <- target_er-div
      ef <-efficient.portfolio(er, covmat, target_er, shorts)
      div <- div/10
